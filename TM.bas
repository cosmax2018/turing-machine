...
'TM Turing Machine fatta in QBasic (compilata con FreeBasic   'fbc -lang qb tm.bas' )
...
'consente di eseguire delle quintuple caricate da file o generate a caso al momento

'ex: > TM -tape tape.t -quint add.tm -b "B" -q0 "0" -qf "8" -t "01XYB" -s "01" -q "0123456789" -m "CRL" -debug
'ex: > TM -quint random-quintuples.tm -b "." -s ".1AB" -q "0123456789" -m "HRL" -rnd -no-exec -debug
'    > TM -h  (to have an help!)
...
dim as integer				WANT_RND_QUINTUPLES : WANT_RND_QUINTUPLES	= FALSE					'want random quintuples?
dim as integer				EXEC				: EXEC					= TRUE					'executes the quintuples
...
if WANT_RND_QUINTUPLES then
	'generates the random quintuples
	gosub QUINTUPLES_GetRnd										'generates rnd quintuples
	gosub QUINTUPLES_ToFile										'save them to file
else
	...
	do
		Select Case UCase$(command$(i))
			...
			Case "-H"
				...
				print "     -RND                           says that you want some random quintuples instead of loading them from a file"
				print "     -NO-EXEC                       inhibits the execution of the quintuples (usefull when they are randomly generated)"
...
QUINTUPLES_GetRnd:			'generates some random quintuples
	'genera alcune quintuple in modo casuale
