...
'TM Turing Machine fatta in QBasic (compilata con FreeBasic   'fbc -lang qb tm-rnd.bas' )
...
'consente di creare ed eseguire delle quintuple generate a caso al momento

'ex: > TM-RND -tape tape.t -quint random-quintuples.tm -b "B" -q0 "0" -qf "8" -t "01XYB" -s "01" -q "0123456789" -m "CRL" -debug    (le crea rnd e le esegue)
'ex: > TM-RND -quint random-quintuples.tm -b "." -s ".1AB" -q "0123456789" -m "HRL" -no-exec -debug                                 (le crea rnd e non le esegue)
'    > TM-RND -h  (to have an help!)
...
if EXEC and DISPLAY then print "final tape configuration:" : gosub TAPE_PrintTape
if EXEC then
